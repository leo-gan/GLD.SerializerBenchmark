...
**Stale vs current harness (as of branch `fix/protobuf-google`):** this snapshot predates the Google **libprotobuf** C row (`protobuf`) and the log rename of in-tree wire to **`protobuf-wire`** (tables may still say `upb:wire-1.0`). Inventory and caveats on the [C overview](index.md) are authoritative; regenerate with `analyze-benchmarks -l c` after a fresh full run.
