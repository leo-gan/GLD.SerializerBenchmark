            ...
            if _cursor < _end {
                ...
                if Int(_tag1) >> 1 == 1 {
                    ...
                    if _tag1 & 1 == 0 {
                        ...
                        _v1 = Int64(_v.fromZigZag); _cursor += _vB
                    } else {
                        _v1 = try Int64.restore(from: data, at: _cursor); _cursor += 8
                    }
                }
            }
            if _cursor < _end {
                ...
                if Int(_tag2) >> 1 == 2 {
                    _cursor += _tag2B
                    let (_abl2, _ablB2) = try restoreLEB(from: data, at: _cursor)
                    _r2 = (_cursor + _ablB2) ..< (_cursor + _ablB2 + Int(_abl2))
                    _cursor += _ablB2 + Int(_abl2)
                }
            }
            if _cursor < _end {
