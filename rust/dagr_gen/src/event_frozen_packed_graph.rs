...
pub use crate::event_frozen_packed_graph_core::*;
...
// ── EventFrozenPackedGraphGraph ─────────────────────────────────────────────────────────────────────
pub trait EventFrozenPackedGraphGraph: EventAttrArena + EventArena {
...
impl<T: EventAttrArena + EventArena> EventFrozenPackedGraphGraph for T {}

// ── EventAttr handle ────────────────────────────────────────────────────────
pub struct EventAttr<'arena, G: EventFrozenPackedGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: EventFrozenPackedGraphGraph> Clone for EventAttr<'arena, G> {
    fn clone(&self) -> Self { EventAttr { index: self.index, graph: self.graph } }
}
impl<'arena, G: EventFrozenPackedGraphGraph> Copy for EventAttr<'arena, G> {}

impl<'arena, G: EventFrozenPackedGraphGraph> EventAttr<'arena, G> {
    ...
    fn _cycle_eq<B: EventFrozenPackedGraphGraph>(&self, other: &EventAttr<'_, B>,
...
impl<'a, 'b, A: EventFrozenPackedGraphGraph, B: EventFrozenPackedGraphGraph> PartialEq<EventAttr<'b, B>> for EventAttr<'a, A> {
    fn eq(&self, other: &EventAttr<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: EventFrozenPackedGraphGraph> Eq for EventAttr<'arena, G> {}

impl<'arena, G: EventFrozenPackedGraphGraph> std::hash::Hash for EventAttr<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: EventFrozenPackedGraphGraph> fmt::Display for EventAttr<'arena, G> {
...
// ── Event handle ────────────────────────────────────────────────────────
pub struct Event<'arena, G: EventFrozenPackedGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: EventFrozenPackedGraphGraph> Clone for Event<'arena, G> {
    fn clone(&self) -> Self { Event { index: self.index, graph: self.graph } }
}
impl<'arena, G: EventFrozenPackedGraphGraph> Copy for Event<'arena, G> {}

impl<'arena, G: EventFrozenPackedGraphGraph> Event<'arena, G> {
    ...
    fn _cycle_eq<B: EventFrozenPackedGraphGraph>(&self, other: &Event<'_, B>,
        visited: &mut HashSet<((u64, usize, usize), (u64, usize, usize))>) -> bool {
        let pair = (self._id(), other._id());
        if visited.contains(&pair) { return true; }
        visited.insert(pair);
...
impl<'a, 'b, A: EventFrozenPackedGraphGraph, B: EventFrozenPackedGraphGraph> PartialEq<Event<'b, B>> for Event<'a, A> {
    fn eq(&self, other: &Event<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: EventFrozenPackedGraphGraph> Eq for Event<'arena, G> {}

impl<'arena, G: EventFrozenPackedGraphGraph> std::hash::Hash for Event<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: EventFrozenPackedGraphGraph> fmt::Display for Event<'arena, G> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut v = HashSet::new(); self._fmt_with(f, &mut v)
    }
}

// ── EventFrozenPackedGraphArena<const ID: u64> ─────────────────────────────────────────────────────
pub struct EventFrozenPackedGraphArena<const ID: u64> {
    arena_of_event_attr: RefCell<Vec<EventAttrValues>>,
    arena_of_event: RefCell<Vec<EventValues>>,
    root: Cell<Option<NodeRef>>,
}

impl<const ID: u64> EventAttrArena for EventFrozenPackedGraphArena<ID> {
...
impl<const ID: u64> EventArena for EventFrozenPackedGraphArena<ID> {
...
impl<const ID: u64> EventFrozenPackedGraphArena<ID> {
    pub fn new() -> Self {
        EventFrozenPackedGraphArena {
...
impl<const ID: u64> Default for EventFrozenPackedGraphArena<ID> {
...
impl<'arena, G: EventFrozenPackedGraphGraph> EventAttr<'arena, G> {
    ...
    pub fn store_packed(&self, b: &mut DagrBuilder) -> Result<NodeStoreRef, DagrError> {
        ...
        let mut _obs0 = 0u8;
        _obs0 |= u8::from(_pr.key.is_some()) << 0;
        _obs0 |= u8::from(_pr.value.is_some()) << 1;
        if let Some(_s_value) = _pr.value.as_deref() {
            let _bs_value = _s_value.as_bytes();
            b.store_raw(_bs_value);
            b.store_leb(_bs_value.len() as u64);
        }
        if let Some(_s_key) = _pr.key.as_deref() {
            let _bs_key = _s_key.as_bytes();
            b.store_raw(_bs_key);
            b.store_leb(_bs_key.len() as u64);
        }
        b.store_u8(_obs0);
...
impl<'arena, G: EventFrozenPackedGraphGraph> Event<'arena, G> {
    ...
    pub fn store_packed(&self, b: &mut DagrBuilder) -> Result<NodeStoreRef, DagrError> {
        ...
        let _pr = &_pr_row[self.index as usize];
        let mut _obs0 = 0u8;
        _obs0 |= u8::from(_pr.event_id.is_some()) << 0;
        _obs0 |= u8::from(_pr.event_type.is_some()) << 1;
        _obs0 |= u8::from(self.occurred_at().is_some()) << 2;
        _obs0 |= u8::from(_pr.producer.is_some()) << 3;
        _obs0 |= u8::from(!self.attrs().is_empty()) << 4;
        let mut _ebs0 = 0u8;
        if let Some(_v_occurred_at) = self.occurred_at() { _ebs0 |= u8::from(crate::dagr_runtime::leb_length(crate::dagr_runtime::to_zigzag(_v_occurred_at as i64)) >= 8) << 0; }
        {
            let _attrs_arr = self.attrs();
            if !_attrs_arr.is_empty() {
                let _cnt_attrs = _attrs_arr.len();
                let _bef_attrs = b.cursor();
                for _e in _attrs_arr.iter().rev() {
                    _e.store_packed(b)?;
                }
        ...
        if let Some(_s_producer) = _pr.producer.as_deref() {
            let _bs_producer = _s_producer.as_bytes();
            b.store_raw(_bs_producer);
            b.store_leb(_bs_producer.len() as u64);
        }
        if let Some(_v_occurred_at) = self.occurred_at() { if _ebs0 & 1 != 0 { b.store_i64(_v_occurred_at); } else { b.store_leb(crate::dagr_runtime::to_zigzag(_v_occurred_at as i64)); } }
        if let Some(_s_event_type) = _pr.event_type.as_deref() {
            let _bs_event_type = _s_event_type.as_bytes();
            b.store_raw(_bs_event_type);
            b.store_leb(_bs_event_type.len() as u64);
        }
        if let Some(_s_event_id) = _pr.event_id.as_deref() {
            let _bs_event_id = _s_event_id.as_bytes();
            b.store_raw(_bs_event_id);
            b.store_leb(_bs_event_id.len() as u64);
        }
        b.store_u8(_ebs0);
        b.store_u8(_obs0);
        b.store_leb((b.cursor() - _before) as u64);
...
fn _restore_event_attr<'arena, G: EventFrozenPackedGraphGraph>(
    ...
    let _value_val = if _obs0 & 2 != 0 {
        let (_sv_value, _slb_value) = crate::dagr_runtime::read_leb(data, _cur)?;
        _cur += _slb_value;
        let _s_value = String::from_utf8_lossy(data.get(_cur.._cur + _sv_value as usize).ok_or(DagrError::InvalidData)?).into_owned();
        _cur += _sv_value as usize;
        Some(_s_value)
    } else { None };
...
fn _restore_event<'arena, G: EventFrozenPackedGraphGraph>(
    data: &'arena [u8], at: usize, arena: &'arena G,
    cache: &mut std::collections::HashMap<(u64, usize), u32>,
    ...
    let _event_id_val = if _obs0 & 1 != 0 {
        let (_sv_event_id, _slb_event_id) = crate::dagr_runtime::read_leb(data, _cur)?;
        _cur += _slb_event_id;
        let _s_event_id = String::from_utf8_lossy(data.get(_cur.._cur + _sv_event_id as usize).ok_or(DagrError::InvalidData)?).into_owned();
        _cur += _sv_event_id as usize;
        Some(_s_event_id)
    } else { None };
    let _event_type_val = if _obs0 & 2 != 0 {
        let (_sv_event_type, _slb_event_type) = crate::dagr_runtime::read_leb(data, _cur)?;
        _cur += _slb_event_type;
        let _s_event_type = String::from_utf8_lossy(data.get(_cur.._cur + _sv_event_type as usize).ok_or(DagrError::InvalidData)?).into_owned();
        _cur += _sv_event_type as usize;
        Some(_s_event_type)
    } else { None };
    let _occurred_at_val = if _obs0 & 4 != 0 {
    ...
    let _producer_val = if _obs0 & 8 != 0 {
        let (_sv_producer, _slb_producer) = crate::dagr_runtime::read_leb(data, _cur)?;
        _cur += _slb_producer;
        let _s_producer = String::from_utf8_lossy(data.get(_cur.._cur + _sv_producer as usize).ok_or(DagrError::InvalidData)?).into_owned();
        _cur += _sv_producer as usize;
        Some(_s_producer)
    } else { None };
    let _attrs_val = if _obs0 & 16 != 0 {
        let (_bsz_attrs, _blb_attrs) = crate::dagr_runtime::read_leb(data, _cur)?;
        _cur += _blb_attrs;
        let _arr_end_attrs = _cur + _bsz_attrs as usize;
        let (_cnt_attrs, _clb_attrs) = crate::dagr_runtime::read_leb(data, _cur)?;
        _cur += _clb_attrs;
        let mut _arr_attrs = Vec::with_capacity(_cnt_attrs as usize);
        for _ in 0.._cnt_attrs {
            let (_esz, _elb) = crate::dagr_runtime::read_leb(data, _cur)?;
            let _eat = _cur; _cur += _elb + _esz as usize;
            _arr_attrs.push(_restore_event_attr(data, _eat, arena, cache)?);
        }
        _cur = _arr_end_attrs;
        _arr_attrs
    } else { vec![] };
