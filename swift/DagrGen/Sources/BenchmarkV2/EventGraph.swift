...
public enum EventGraph {
    ...
    //
    //   enum MyBrand {}
    //   let arena = EventGraph.Arena<MyBrand>()
    //
...
extension EventGraph.Arena {
    public func adopt<S: EventGraph.EventAttrGraph>(_ src: EventGraph.EventAttr<S>) throws -> EventGraph.EventAttr<EventGraph.Arena<Brand>> {
        var _seen = Set<UInt64>()
        return try _adopt(src, &_seen)
    }
    func _adopt<S: EventGraph.EventAttrGraph>(_ src: EventGraph.EventAttr<S>, _ _seen: inout Set<UInt64>) throws -> EventGraph.EventAttr<EventGraph.Arena<Brand>> {
...
extension EventGraph.Arena {
    public func adopt<S: EventGraph.EventGraph>(_ src: EventGraph.Event<S>) throws -> EventGraph.Event<EventGraph.Arena<Brand>> {
        var _seen = Set<UInt64>()
        return try _adopt(src, &_seen)
    }
    func _adopt<S: EventGraph.EventGraph>(_ src: EventGraph.Event<S>, _ _seen: inout Set<UInt64>) throws -> EventGraph.Event<EventGraph.Arena<Brand>> {
...
extension EventGraph.EventAttr: CustomStringConvertible {
...
extension EventGraph.Event: CustomStringConvertible {
    public var description: String {
        var visited = Set<NodeKey>()
        return buildDescription(visited: &visited)
    }
...
extension EventGraph.EventAttr: Hashable {
...
extension EventGraph.Event: Hashable {
    public func hash(into hasher: inout Hasher) {
        var visited = Set<NodeKey>()
        hashInto(hasher: &hasher, visited: &visited)
    }
...
extension EventGraph.EventAttr: Equatable {
...
extension EventGraph.EventAttr {
    public static func == <OtherArena: EventGraph.EventAttrGraph>(lhs: Self, rhs: EventGraph.EventAttr<OtherArena>) -> Bool {
        var visited = Set<ArenaPair>()
    ...
    func cycleAwareEqualsAny<OtherArena: EventGraph.EventAttrGraph>(
        other: EventGraph.EventAttr<OtherArena>,
...
extension EventGraph.Event: Equatable {
    public static func == (lhs: Self, rhs: Self) -> Bool {
        var visited = Set<ArenaPair>()
        return lhs.cycleAwareEquals(other: rhs, visited: &visited)
    }
...
extension EventGraph.Event {
    public static func == <OtherArena: EventGraph.EventGraph>(lhs: Self, rhs: EventGraph.Event<OtherArena>) -> Bool {
        var visited = Set<ArenaPair>()
        return lhs.cycleAwareEqualsAny(other: rhs, visited: &visited)
    }
    ...
    func cycleAwareEqualsAny<OtherArena: EventGraph.EventGraph>(
        other: EventGraph.Event<OtherArena>,
        visited: inout Set<ArenaPair>
    ) -> Bool {
        let pair = ArenaPair(
            leftArena: _arenaId, leftTypeId: Arena.eventTypeId, leftIndex: __index,
...
extension EventGraph.EventAttr: ArenaNodeHandle {}
extension EventGraph.EventAttr: ArenaGraphStorable {
...
extension EventGraph.Event: ArenaNodeHandle {}
extension EventGraph.Event: ArenaGraphStorable {
    var cycleIdentifier: ArenaCycleIdntifier {
...
extension EventGraph.Arena {
    ...
    public static func restore(from data: Foundation.Data) throws -> EventGraph.Arena<Brand> {
        guard !data.isEmpty else { return EventGraph.Arena<Brand>() }
        let arena = EventGraph.Arena<Brand>()
    ...
    private func _restoreEventAttr(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> EventGraph.EventAttr<EventGraph.Arena<Brand>> {
        if let idx = cache[start] { return EventGraph.EventAttr(__packed: UInt64(idx), __graph: self) }
        ...
        return EventGraph.EventAttr(__packed: UInt64(idx), __graph: self)
    }

    private func _restoreEvent(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> EventGraph.Event<EventGraph.Arena<Brand>> {
        if let idx = cache[start] { return EventGraph.Event(__packed: UInt64(idx), __graph: self) }
        ...
        if _cursor < _end {
            ...
            if Int(_tag) >> 1 == 4 {
                ...
                for _ in 0..<Int(_cnt_attrs) {
                    ...
                    let _nd_attrs = try _restoreEventAttr(from: data, at: _pos_attrs, cache: &cache)
        ...
        return EventGraph.Event(__packed: UInt64(idx), __graph: self)
    }
