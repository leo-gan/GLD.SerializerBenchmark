...
def generate_markdown_summary(
    ...
    **_kwargs,
    ...
    lines.append(
        "\nPublished on the documentation site as a **local snapshot** under "
        "`docs/analysis/` (not regenerated by GitHub Actions). Re-running "
        "benchmarks on another machine may yield different numbers — that is OK.\n"
    )
    lines.append("---\n\n")
...
def write_violin_plots_markdown(
    ...
    lines = [
        ...
        "(wider = more samples at that duration). Generated **locally** by "
        "`analyze-benchmarks --generate-plots` into `docs/analysis/plots/violin/` for the "
        "documentation site (not by GitHub Actions). Re-running benchmarks elsewhere "
        "may produce different shapes — that is expected and OK.",
        "",
