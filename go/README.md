...
| dagr-packed | Schema | `gen/dagrv2` (generated by `dagr build`, local module via `replace`) | generated direct builder (`<Graph>Builder.BuildAppend`, one builder reused) / lazy accessors → domain value, both timed; N>1 framed `u32 count + (u32 len + record)×N`; **stream adapted** |
| dagr-regular | Schema | `gen/dagrv2/<type>regulargraph` (same `dagr build`) | regular (vtable) nodes: generated arena built in `Prepare` (untimed), `AppendTo<Graph>(b, dst, root)` into one reused `dagr.Builder` timed / lazy accessors → domain value timed; framing and stream as `dagr-packed` |
