...
| dagr | Schema | `gen/dagrv2` (generated by `dagr build`, local module via `replace`) | generated direct builder (`<Graph>Builder.BuildAppend`, one builder reused) / lazy accessors → domain value, both timed; N>1 framed `u32 count + (u32 len + record)×N`; **stream adapted** |
| dagr-regular | Schema | `gen/dagrv2/<type>regulargraph` (same `dagr build`) | regular (vtable) nodes: generated arena built in `Prepare` (untimed), `ToBytes<Graph>(root, 0)` timed / lazy accessors → domain value timed; framing and stream as `dagr` |
| dagr-frozen | Schema | `gen/dagrv2/<type>frozengraph` (same `dagr build`) | frozen (fixed-struct) nodes: generated arena built in `Prepare` (untimed), `ToBytes<Graph>(root, 0)` timed / lazy accessors → domain value timed; framing and stream as `dagr` |
| dagr-frozen-packed | Schema | `gen/dagrv2/<type>frozenpackedgraphdirect` (same `dagr build`) | frozen+packed nodes: same call path as `dagr` (direct builder `BuildAppend`, one builder reused / lazy accessors); framing and stream as `dagr` |
