#!/usr/bin/env bash
# Experiment 6 — run one language or every language.
# Does not refresh the published website tables.
#
#   ./experiments/06-document-db-formats/run.sh
#   ./experiments/06-document-db-formats/run.sh java go
set -euo pipefail
...
run_one() {
  ...
  if [[ -z "$script" ]]; then
    echo "[exp-06] unknown language: $lang" >&2
    return 1
  fi
  if [[ ! -f "$script" ]]; then
    echo "[exp-06] missing runner: $script" >&2
    return 1
  fi
  ...
  if LOG_DIR="$log_dir" BENCHMARK_RUN_CONFIG="$HERE/run.yaml" \
    ...
    if [[ -z "$csv" ]]; then
      echo "[exp-06] $lang: runner finished but no CSV" >&2
      return 1
    fi
    (
      cd "$REPO/analysis"
    ...
    echo "[exp-06] $lang: runner failed" >&2
    return 1
  fi
}
...
if [[ ${#failed[@]} -gt 0 ]]; then
  echo "[exp-06] failed: ${failed[*]}" >&2
  exit 1
fi
echo "[exp-06] all requested languages finished"
