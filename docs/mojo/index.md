...
| mojo-json | JSON | leo-gan/gld-json 0.4.0 | bytes only | Typed WireWriter / WireReader (vendored as `gldjson`) |
| mojo-cbor | Binary | leo-gan/gld-cbor 0.7.0 | bytes only | `CborDatum` encode / decode |
...
| gld-yaml | Text | [leo-gan/gld-yaml](https://github.com/leo-gan/gld-yaml) 0.4.0 | bytes only | `yaml.encode` / `yaml.decode` on suite types |
