...
// ── EventFrozenGraphGraph ─────────────────────────────────────────────────────────────────────
pub trait EventFrozenGraphGraph: EventAttrArena + EventArena {
...
impl<T: EventAttrArena + EventArena> EventFrozenGraphGraph for T {}

// ── EventAttr handle ────────────────────────────────────────────────────────
pub struct EventAttr<'arena, G: EventFrozenGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: EventFrozenGraphGraph> Clone for EventAttr<'arena, G> {
    fn clone(&self) -> Self { EventAttr { index: self.index, graph: self.graph } }
}
impl<'arena, G: EventFrozenGraphGraph> Copy for EventAttr<'arena, G> {}

impl<'arena, G: EventFrozenGraphGraph> EventAttr<'arena, G> {
    ...
    fn _cycle_eq<B: EventFrozenGraphGraph>(&self, other: &EventAttr<'_, B>,
...
impl<'a, 'b, A: EventFrozenGraphGraph, B: EventFrozenGraphGraph> PartialEq<EventAttr<'b, B>> for EventAttr<'a, A> {
    fn eq(&self, other: &EventAttr<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: EventFrozenGraphGraph> Eq for EventAttr<'arena, G> {}

impl<'arena, G: EventFrozenGraphGraph> std::hash::Hash for EventAttr<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: EventFrozenGraphGraph> fmt::Display for EventAttr<'arena, G> {
...
// ── Event handle ────────────────────────────────────────────────────────
pub struct Event<'arena, G: EventFrozenGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: EventFrozenGraphGraph> Clone for Event<'arena, G> {
    fn clone(&self) -> Self { Event { index: self.index, graph: self.graph } }
}
impl<'arena, G: EventFrozenGraphGraph> Copy for Event<'arena, G> {}

impl<'arena, G: EventFrozenGraphGraph> Event<'arena, G> {
    ...
    fn _cycle_eq<B: EventFrozenGraphGraph>(&self, other: &Event<'_, B>,
        visited: &mut HashSet<((u64, usize, usize), (u64, usize, usize))>) -> bool {
        let pair = (self._id(), other._id());
        if visited.contains(&pair) { return true; }
        visited.insert(pair);
...
impl<'a, 'b, A: EventFrozenGraphGraph, B: EventFrozenGraphGraph> PartialEq<Event<'b, B>> for Event<'a, A> {
    fn eq(&self, other: &Event<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: EventFrozenGraphGraph> Eq for Event<'arena, G> {}

impl<'arena, G: EventFrozenGraphGraph> std::hash::Hash for Event<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: EventFrozenGraphGraph> fmt::Display for Event<'arena, G> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut v = HashSet::new(); self._fmt_with(f, &mut v)
    }
}

// ── EventFrozenGraphArena<const ID: u64> ─────────────────────────────────────────────────────
pub struct EventFrozenGraphArena<const ID: u64> {
    arena_of_event_attr: RefCell<Vec<EventAttrValues>>,
    arena_of_event: RefCell<Vec<EventValues>>,
    root: Cell<Option<NodeRef>>,
}

impl<const ID: u64> EventAttrArena for EventFrozenGraphArena<ID> {
...
impl<const ID: u64> EventArena for EventFrozenGraphArena<ID> {
...
impl<const ID: u64> EventFrozenGraphArena<ID> {
    pub fn new() -> Self {
        EventFrozenGraphArena {
...
impl<const ID: u64> Default for EventFrozenGraphArena<ID> {
...
impl<'arena, G: EventFrozenGraphGraph> EventAttr<'arena, G> {
    pub fn store(&self, b: &mut DagrBuilder) -> Result<NodeStoreRef, DagrError> {
        ...
        if let Some(off) = _value_off { b.store_forward_pointer(off); }
        if let Some(off) = _key_off { b.store_forward_pointer(off); }
        b.store_u8(u8::from(_pr.key.is_some()) | (u8::from(_pr.value.is_some()) << 1));
...
impl<'arena, G: EventFrozenGraphGraph> Event<'arena, G> {
    pub fn store(&self, b: &mut DagrBuilder) -> Result<NodeStoreRef, DagrError> {
        ...
        if let Some(off) = _attrs_off { b.store_forward_pointer(off); }
        if let Some(off) = _producer_off { b.store_forward_pointer(off); }
        if let Some(_v_occurred_at) = self.occurred_at() { b.store_i64(_v_occurred_at); }
        if let Some(off) = _event_type_off { b.store_forward_pointer(off); }
        if let Some(off) = _event_id_off { b.store_forward_pointer(off); }
        b.store_u8(u8::from(_pr.event_id.is_some()) | (u8::from(_pr.event_type.is_some()) << 1) | (u8::from(self.occurred_at().is_some()) << 2) | (u8::from(_pr.producer.is_some()) << 3) | (u8::from(!self.attrs().is_empty()) << 4));
        let _offset = b.cursor();
        b.record_node_offset(_id, _offset);
        Ok(NodeStoreRef::Offset(_offset))
    }
...
fn _restore_event_attr<'arena, G: EventFrozenGraphGraph>(
...
fn _restore_event<'arena, G: EventFrozenGraphGraph>(
    data: &'arena [u8], at: usize, arena: &'arena G,
    cache: &mut std::collections::HashMap<(u64, usize), u32>,
    ...
    let _attrs_val = if _cur < data.len() {
        let (_fwd_raw, _fwd_bs) = crate::dagr_runtime::read_v62(data, _cur)?;
        let _at_attrs = _cur + _fwd_bs + _fwd_raw as usize;
        _cur += _fwd_bs;
        let _items: Result<Vec<_>, DagrError> = crate::dagr_runtime::read_node_ref_array(data, _at_attrs)?.into_iter()
