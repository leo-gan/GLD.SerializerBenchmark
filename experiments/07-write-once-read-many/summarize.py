...
Rebuild from saved CSVs (no new timing run):
    ...
    cd analysis
    uv run python ../experiments/07-write-once-read-many/summarize.py --all

One language:

    uv run python ../experiments/07-write-once-read-many/summarize.py \\
...
def write_language_markdown(language: str, pack: dict) -> str:
    lines = [
        f"# Experiment 7 results — {language}",
        "",
