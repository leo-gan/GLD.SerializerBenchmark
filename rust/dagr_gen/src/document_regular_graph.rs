...
// ── DocumentRegularGraphGraph ─────────────────────────────────────────────────────────────────────
pub trait DocumentRegularGraphGraph: DocumentMetaArena + DocumentItemArena + DocumentArena {
...
impl<T: DocumentMetaArena + DocumentItemArena + DocumentArena> DocumentRegularGraphGraph for T {}

// ── DocumentMeta handle ────────────────────────────────────────────────────────
pub struct DocumentMeta<'arena, G: DocumentRegularGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: DocumentRegularGraphGraph> Clone for DocumentMeta<'arena, G> {
    fn clone(&self) -> Self { DocumentMeta { index: self.index, graph: self.graph } }
}
impl<'arena, G: DocumentRegularGraphGraph> Copy for DocumentMeta<'arena, G> {}

impl<'arena, G: DocumentRegularGraphGraph> DocumentMeta<'arena, G> {
    ...
    fn _cycle_eq<B: DocumentRegularGraphGraph>(&self, other: &DocumentMeta<'_, B>,
...
impl<'a, 'b, A: DocumentRegularGraphGraph, B: DocumentRegularGraphGraph> PartialEq<DocumentMeta<'b, B>> for DocumentMeta<'a, A> {
    fn eq(&self, other: &DocumentMeta<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: DocumentRegularGraphGraph> Eq for DocumentMeta<'arena, G> {}

impl<'arena, G: DocumentRegularGraphGraph> std::hash::Hash for DocumentMeta<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: DocumentRegularGraphGraph> fmt::Display for DocumentMeta<'arena, G> {
...
// ── DocumentItem handle ────────────────────────────────────────────────────────
pub struct DocumentItem<'arena, G: DocumentRegularGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: DocumentRegularGraphGraph> Clone for DocumentItem<'arena, G> {
    fn clone(&self) -> Self { DocumentItem { index: self.index, graph: self.graph } }
}
impl<'arena, G: DocumentRegularGraphGraph> Copy for DocumentItem<'arena, G> {}

impl<'arena, G: DocumentRegularGraphGraph> DocumentItem<'arena, G> {
    ...
    fn _cycle_eq<B: DocumentRegularGraphGraph>(&self, other: &DocumentItem<'_, B>,
        visited: &mut HashSet<((u64, usize, usize), (u64, usize, usize))>) -> bool {
        let pair = (self._id(), other._id());
        if visited.contains(&pair) { return true; }
        visited.insert(pair);
...
impl<'a, 'b, A: DocumentRegularGraphGraph, B: DocumentRegularGraphGraph> PartialEq<DocumentItem<'b, B>> for DocumentItem<'a, A> {
    fn eq(&self, other: &DocumentItem<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: DocumentRegularGraphGraph> Eq for DocumentItem<'arena, G> {}

impl<'arena, G: DocumentRegularGraphGraph> std::hash::Hash for DocumentItem<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: DocumentRegularGraphGraph> fmt::Display for DocumentItem<'arena, G> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut v = HashSet::new(); self._fmt_with(f, &mut v)
    }
}
...
// ── Document handle ────────────────────────────────────────────────────────
pub struct Document<'arena, G: DocumentRegularGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: DocumentRegularGraphGraph> Clone for Document<'arena, G> {
    fn clone(&self) -> Self { Document { index: self.index, graph: self.graph } }
}
impl<'arena, G: DocumentRegularGraphGraph> Copy for Document<'arena, G> {}

impl<'arena, G: DocumentRegularGraphGraph> Document<'arena, G> {
    ...
    fn _cycle_eq<B: DocumentRegularGraphGraph>(&self, other: &Document<'_, B>,
        visited: &mut HashSet<((u64, usize, usize), (u64, usize, usize))>) -> bool {
        let pair = (self._id(), other._id());
        if visited.contains(&pair) { return true; }
        visited.insert(pair);
...
impl<'a, 'b, A: DocumentRegularGraphGraph, B: DocumentRegularGraphGraph> PartialEq<Document<'b, B>> for Document<'a, A> {
    fn eq(&self, other: &Document<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: DocumentRegularGraphGraph> Eq for Document<'arena, G> {}

impl<'arena, G: DocumentRegularGraphGraph> std::hash::Hash for Document<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: DocumentRegularGraphGraph> fmt::Display for Document<'arena, G> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut v = HashSet::new(); self._fmt_with(f, &mut v)
    }
}

// ── DocumentRegularGraphArena<const ID: u64> ─────────────────────────────────────────────────────
pub struct DocumentRegularGraphArena<const ID: u64> {
...
impl<const ID: u64> DocumentMetaArena for DocumentRegularGraphArena<ID> {
...
impl<const ID: u64> DocumentItemArena for DocumentRegularGraphArena<ID> {
...
impl<const ID: u64> DocumentArena for DocumentRegularGraphArena<ID> {
...
impl<const ID: u64> DocumentRegularGraphArena<ID> {
    pub fn new() -> Self {
...
impl<const ID: u64> Default for DocumentRegularGraphArena<ID> {
...
impl<'arena, G: DocumentRegularGraphGraph> DocumentMeta<'arena, G> {
    pub fn store(&self, b: &mut DagrBuilder) -> Result<NodeStoreRef, DagrError> {
        ...
        let _region_ptr = _region_off.map(|off| b.store_forward_pointer(off));
        let _offset = b.store_vtable(&[_region_ptr, _version_ptr]);
...
impl<'arena, G: DocumentRegularGraphGraph> DocumentItem<'arena, G> {
    pub fn store(&self, b: &mut DagrBuilder) -> Result<NodeStoreRef, DagrError> {
        ...
        let _price_minor_ptr = self.price_minor().map(|v| b.store_i64(v));
        let _qty_ptr = self.qty().map(|v| b.store_i32(v));
        let _sku_ptr = _sku_off.map(|off| b.store_forward_pointer(off));
        let _offset = b.store_vtable(&[_sku_ptr, _qty_ptr, _price_minor_ptr]);
        b.record_node_offset(_id, _offset);
        Ok(NodeStoreRef::Offset(_offset))
    }
...
impl<'arena, G: DocumentRegularGraphGraph> Document<'arena, G> {
    pub fn store(&self, b: &mut DagrBuilder) -> Result<NodeStoreRef, DagrError> {
        ...
        let _meta_ptr = _meta_off.map(|r| b.store_bidir_pointer(r));
        let _status_ptr = self.status().map(|v| b.store_i32(v));
        let _id_ptr = _id_off.map(|off| b.store_forward_pointer(off));
        let _offset = b.store_vtable(&[_id_ptr, _status_ptr, _meta_ptr, _items_ptr]);
        b.record_node_offset(_id, _offset);
        Ok(NodeStoreRef::Offset(_offset))
    }
...
fn _restore_document_meta<'arena, G: DocumentRegularGraphGraph>(
    ...
    let _region_val = _slots.slot(0).map(|fwd| -> Result<String, DagrError> { Ok(crate::dagr_runtime::read_string(data, crate::dagr_runtime::read_forward_pointer(data, fwd)?)?) }).transpose()?;
    let _version_val = _slots.slot(1).map(|at| crate::dagr_runtime::read_i32(data, at).unwrap_or_default() as i32);
    _node.set_region(_region_val.as_deref());
    _node.set_version(_version_val);
    Ok(_node)
}

fn _restore_document_item<'arena, G: DocumentRegularGraphGraph>(
    data: &'arena [u8], at: usize, arena: &'arena G,
    cache: &mut std::collections::HashMap<(u64, usize), u32>,
    ...
    let _price_minor_val = _slots.slot(2).map(|at| crate::dagr_runtime::read_i64(data, at).unwrap_or_default() as i64);
...
fn _restore_document<'arena, G: DocumentRegularGraphGraph>(
    data: &'arena [u8], at: usize, arena: &'arena G,
    cache: &mut std::collections::HashMap<(u64, usize), u32>,
    ...
    cache.insert(_cache_key, _node.index);
    let _slots = crate::dagr_runtime::read_vtable(data, at)?;
    let _id_val = _slots.slot(0).map(|fwd| -> Result<String, DagrError> { Ok(crate::dagr_runtime::read_string(data, crate::dagr_runtime::read_forward_pointer(data, fwd)?)?) }).transpose()?;
    let _status_val = _slots.slot(1).map(|at| crate::dagr_runtime::read_i32(data, at).unwrap_or_default() as i32);
    let _meta_val = _slots.slot(2).map(|bp_at| _restore_document_meta(data, crate::dagr_runtime::read_bidir_pointer(data, bp_at)?, arena, cache)).transpose()?;
    let _items_val = if let Some(fwd) = _slots.slot(3) {
