...
public enum DocumentFrozenGraph {
    ...
    public typealias DocumentFrozenGraphGraph = DocumentArena & DocumentItemArena & DocumentMetaArena
    ...
    //
    //   enum MyBrand {}
    //   let arena = DocumentFrozenGraph.Arena<MyBrand>()
    //
...
extension DocumentFrozenGraph.Arena {
    public func newDocumentMeta(region: String? = nil, version: Int32? = nil) -> DocumentFrozenGraph.DocumentMeta<DocumentFrozenGraph.Arena<Brand>> {
        let _idx: Int
        _idx = arenaOfDocumentMeta.count
        arenaOfDocumentMeta.append(DocumentFrozenGraph.DocumentMetaValues(region: region, version: version))
        let _packed = UInt64(_idx)
        return DocumentFrozenGraph.DocumentMeta(__packed: _packed, __graph: self)
    }
}

extension DocumentFrozenGraph.Arena {
    public func newDocumentItem(sku: String? = nil, qty: Int32? = nil, price_minor: Int64? = nil) -> DocumentFrozenGraph.DocumentItem<DocumentFrozenGraph.Arena<Brand>> {
        let _idx: Int
        _idx = arenaOfDocumentItem.count
        arenaOfDocumentItem.append(DocumentFrozenGraph.DocumentItemValues(sku: sku, qty: qty, price_minor: price_minor))
        let _packed = UInt64(_idx)
        return DocumentFrozenGraph.DocumentItem(__packed: _packed, __graph: self)
    }
}

extension DocumentFrozenGraph.Arena {
    public func newDocument(id: String? = nil, status: Int32? = nil, meta: DocumentFrozenGraph.DocumentMeta<DocumentFrozenGraph.Arena<Brand>>? = nil, items: [DocumentFrozenGraph.DocumentItem<DocumentFrozenGraph.Arena<Brand>>] = []) -> DocumentFrozenGraph.Document<DocumentFrozenGraph.Arena<Brand>> {
        let _idx: Int
        _idx = arenaOfDocument.count
        arenaOfDocument.append(DocumentFrozenGraph.DocumentValues(id: id, status: status, meta: meta?.__packed, items: items.map { $0.__packed }))
        let _packed = UInt64(_idx)
        return DocumentFrozenGraph.Document(__packed: _packed, __graph: self)
    }
}

extension DocumentFrozenGraph.Arena {
    public func adopt<S: DocumentFrozenGraph.DocumentMetaGraph>(_ src: DocumentFrozenGraph.DocumentMeta<S>) throws -> DocumentFrozenGraph.DocumentMeta<DocumentFrozenGraph.Arena<Brand>> {
        var _seen = Set<UInt64>()
        return try _adopt(src, &_seen)
    }
    func _adopt<S: DocumentFrozenGraph.DocumentMetaGraph>(_ src: DocumentFrozenGraph.DocumentMeta<S>, _ _seen: inout Set<UInt64>) throws -> DocumentFrozenGraph.DocumentMeta<DocumentFrozenGraph.Arena<Brand>> {
...
extension DocumentFrozenGraph.Arena {
    public func adopt<S: DocumentFrozenGraph.DocumentItemGraph>(_ src: DocumentFrozenGraph.DocumentItem<S>) throws -> DocumentFrozenGraph.DocumentItem<DocumentFrozenGraph.Arena<Brand>> {
        var _seen = Set<UInt64>()
        return try _adopt(src, &_seen)
    }
    func _adopt<S: DocumentFrozenGraph.DocumentItemGraph>(_ src: DocumentFrozenGraph.DocumentItem<S>, _ _seen: inout Set<UInt64>) throws -> DocumentFrozenGraph.DocumentItem<DocumentFrozenGraph.Arena<Brand>> {
...
extension DocumentFrozenGraph.Arena {
    public func adopt<S: DocumentFrozenGraph.DocumentGraph>(_ src: DocumentFrozenGraph.Document<S>) throws -> DocumentFrozenGraph.Document<DocumentFrozenGraph.Arena<Brand>> {
        var _seen = Set<UInt64>()
        return try _adopt(src, &_seen)
    }
    func _adopt<S: DocumentFrozenGraph.DocumentGraph>(_ src: DocumentFrozenGraph.Document<S>, _ _seen: inout Set<UInt64>) throws -> DocumentFrozenGraph.Document<DocumentFrozenGraph.Arena<Brand>> {
...
extension DocumentFrozenGraph.DocumentMeta: CustomStringConvertible {
...
extension DocumentFrozenGraph.DocumentItem: CustomStringConvertible {
    public var description: String {
        var visited = Set<NodeKey>()
        return buildDescription(visited: &visited)
    }
...
extension DocumentFrozenGraph.Document: CustomStringConvertible {
    public var description: String {
        var visited = Set<NodeKey>()
        return buildDescription(visited: &visited)
    }
...
extension DocumentFrozenGraph.DocumentMeta: Hashable {
...
extension DocumentFrozenGraph.DocumentItem: Hashable {
    public func hash(into hasher: inout Hasher) {
        var visited = Set<NodeKey>()
        hashInto(hasher: &hasher, visited: &visited)
    }
...
extension DocumentFrozenGraph.Document: Hashable {
    public func hash(into hasher: inout Hasher) {
        var visited = Set<NodeKey>()
        hashInto(hasher: &hasher, visited: &visited)
    }
...
extension DocumentFrozenGraph.DocumentMeta: Equatable {
...
extension DocumentFrozenGraph.DocumentMeta {
    public static func == <OtherArena: DocumentFrozenGraph.DocumentMetaGraph>(lhs: Self, rhs: DocumentFrozenGraph.DocumentMeta<OtherArena>) -> Bool {
        var visited = Set<ArenaPair>()
    ...
    func cycleAwareEqualsAny<OtherArena: DocumentFrozenGraph.DocumentMetaGraph>(
        other: DocumentFrozenGraph.DocumentMeta<OtherArena>,
...
extension DocumentFrozenGraph.DocumentItem: Equatable {
    public static func == (lhs: Self, rhs: Self) -> Bool {
        var visited = Set<ArenaPair>()
        return lhs.cycleAwareEquals(other: rhs, visited: &visited)
    }
...
extension DocumentFrozenGraph.DocumentItem {
    public static func == <OtherArena: DocumentFrozenGraph.DocumentItemGraph>(lhs: Self, rhs: DocumentFrozenGraph.DocumentItem<OtherArena>) -> Bool {
        var visited = Set<ArenaPair>()
        return lhs.cycleAwareEqualsAny(other: rhs, visited: &visited)
    }
    ...
    func cycleAwareEqualsAny<OtherArena: DocumentFrozenGraph.DocumentItemGraph>(
        other: DocumentFrozenGraph.DocumentItem<OtherArena>,
        visited: inout Set<ArenaPair>
    ) -> Bool {
        let pair = ArenaPair(
            leftArena: _arenaId, leftTypeId: Arena.documentItemTypeId, leftIndex: __index,
...
extension DocumentFrozenGraph.Document: Equatable {
    public static func == (lhs: Self, rhs: Self) -> Bool {
        var visited = Set<ArenaPair>()
        return lhs.cycleAwareEquals(other: rhs, visited: &visited)
    }
...
extension DocumentFrozenGraph.Document {
    public static func == <OtherArena: DocumentFrozenGraph.DocumentGraph>(lhs: Self, rhs: DocumentFrozenGraph.Document<OtherArena>) -> Bool {
        var visited = Set<ArenaPair>()
        return lhs.cycleAwareEqualsAny(other: rhs, visited: &visited)
    }
    ...
    func cycleAwareEqualsAny<OtherArena: DocumentFrozenGraph.DocumentGraph>(
        other: DocumentFrozenGraph.Document<OtherArena>,
        visited: inout Set<ArenaPair>
    ) -> Bool {
        let pair = ArenaPair(
            leftArena: _arenaId, leftTypeId: Arena.documentTypeId, leftIndex: __index,
...
extension DocumentFrozenGraph.DocumentMeta: ArenaNodeHandle {}
extension DocumentFrozenGraph.DocumentMeta: ArenaGraphStorable {
    ...
    public func store(with builder: any ArenaBuilder) throws -> BufferOffset {
        ...
        _ = try _regionValueOffset?.storeForwardPointer(with: builder)
...
extension DocumentFrozenGraph.DocumentItem: ArenaNodeHandle {}
extension DocumentFrozenGraph.DocumentItem: ArenaGraphStorable {
    var cycleIdentifier: ArenaCycleIdntifier {
    ...
    public func store(with builder: any ArenaBuilder) throws -> BufferOffset {
        ...
        _ = try self.price_minor?.store(with: builder)
...
extension DocumentFrozenGraph.Document: ArenaNodeHandle {}
extension DocumentFrozenGraph.Document: ArenaGraphStorable {
    var cycleIdentifier: ArenaCycleIdntifier {
    ...
    public func store(with builder: any ArenaBuilder) throws -> BufferOffset {
        ...
        _ = try _metaValueOffset?.storeBidirectionalPointer(with: builder)
        _ = try self.status?.store(with: builder)
        _ = try _idValueOffset?.storeForwardPointer(with: builder)
        var _nilByte: UInt8 = 0
...
extension DocumentFrozenGraph.Arena {
    ...
    public static func restore(from data: Foundation.Data) throws -> DocumentFrozenGraph.Arena<Brand> {
        guard !data.isEmpty else { return DocumentFrozenGraph.Arena<Brand>() }
        let arena = DocumentFrozenGraph.Arena<Brand>()
    ...
    private func _restoreDocumentMeta(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> DocumentFrozenGraph.DocumentMeta<DocumentFrozenGraph.Arena<Brand>> {
        if let idx = cache[start] { return DocumentFrozenGraph.DocumentMeta(__packed: UInt64(idx), __graph: self) }
        let idx = arenaOfDocumentMeta.count
        cache[start] = idx
        arenaOfDocumentMeta.append(DocumentFrozenGraph.DocumentMetaValues())
        var values = DocumentFrozenGraph.DocumentMetaValues()
        let _obs0 = start + 0 < data.count ? data[start + 0] : UInt8(0)
        var _cur = start + 1
        if _obs0 & 1 != 0 {
            let (_fwd_region, _fwdB_region) = try readV62(from: data, at: _cur)
            _cur += _fwdB_region
            values.region = try String.restore(from: data, at: _cur + Int(_fwd_region))
        }
        if _obs0 & 2 != 0 {
        ...
        return DocumentFrozenGraph.DocumentMeta(__packed: UInt64(idx), __graph: self)
    }

    private func _restoreDocumentMetaFrozenPacked(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> DocumentFrozenGraph.DocumentMeta<DocumentFrozenGraph.Arena<Brand>> {
        if let idx = cache[start] { return DocumentFrozenGraph.DocumentMeta(__packed: UInt64(idx), __graph: self) }
        let idx = arenaOfDocumentMeta.count
        cache[start] = idx
        arenaOfDocumentMeta.append(DocumentFrozenGraph.DocumentMetaValues())
    ...
    private func _restoreDocumentItem(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> DocumentFrozenGraph.DocumentItem<DocumentFrozenGraph.Arena<Brand>> {
        if let idx = cache[start] { return DocumentFrozenGraph.DocumentItem(__packed: UInt64(idx), __graph: self) }
        let idx = arenaOfDocumentItem.count
        cache[start] = idx
        arenaOfDocumentItem.append(DocumentFrozenGraph.DocumentItemValues())
        var values = DocumentFrozenGraph.DocumentItemValues()
        let _obs0 = start + 0 < data.count ? data[start + 0] : UInt8(0)
        var _cur = start + 1
        if _obs0 & 1 != 0 {
        ...
        if _obs0 & 4 != 0 {
            values.price_minor = try Int64.restore(from: data, at: _cur)
            _cur += 8
        }
        arenaOfDocumentItem[idx] = values
        return DocumentFrozenGraph.DocumentItem(__packed: UInt64(idx), __graph: self)
    }

    private func _restoreDocumentItemFrozenPacked(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> DocumentFrozenGraph.DocumentItem<DocumentFrozenGraph.Arena<Brand>> {
        if let idx = cache[start] { return DocumentFrozenGraph.DocumentItem(__packed: UInt64(idx), __graph: self) }
        let idx = arenaOfDocumentItem.count
        cache[start] = idx
        arenaOfDocumentItem.append(DocumentFrozenGraph.DocumentItemValues())
    ...
    private func _restoreDocument(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> DocumentFrozenGraph.Document<DocumentFrozenGraph.Arena<Brand>> {
        if let idx = cache[start] { return DocumentFrozenGraph.Document(__packed: UInt64(idx), __graph: self) }
        let idx = arenaOfDocument.count
        cache[start] = idx
        arenaOfDocument.append(DocumentFrozenGraph.DocumentValues())
        var values = DocumentFrozenGraph.DocumentValues()
        let _obs0 = start + 0 < data.count ? data[start + 0] : UInt8(0)
        var _cur = start + 1
        if _obs0 & 1 != 0 {
            let (_fwd_id, _fwdB_id) = try readV62(from: data, at: _cur)
            _cur += _fwdB_id
            values.id = try String.restore(from: data, at: _cur + Int(_fwd_id))
        }
        if _obs0 & 2 != 0 {
            values.status = try Int32.restore(from: data, at: _cur)
            _cur += 4
        }
        if _obs0 & 4 != 0 {
            let (_bd_meta, _bdb_meta) = try readZigZagV62(from: data, at: _cur)
            _cur += _bdb_meta
            values.meta = try _restoreDocumentMeta(from: data, at: _cur + _bd_meta, cache: &cache).__packed
        }
        let (_fwd_items, _fwdB_items) = try readV62(from: data, at: _cur)
        _cur += _fwdB_items
        let (_h_items, _hl_items) = try restoreLEB(from: data, at: _cur + Int(_fwd_items))
        let _cnt_items = Int(_h_items >> 2); let _wc_items = Int(_h_items & 3)
        let _es_items = [1,2,4,8][_wc_items]
        let _base_items = _cur + Int(_fwd_items) + _hl_items + _cnt_items * _es_items
        var _arr_items = [UInt64]()
        for _k_items in 0..<_cnt_items {
            let _ep_items = _cur + Int(_fwd_items) + _hl_items + _k_items * _es_items
            let _ro_items = try readSignedRelOffset(from: data, at: _ep_items, size: _es_items)
            if _ro_items != 0 {
                _arr_items.append(try _restoreDocumentItem(from: data, at: _base_items + Int(_ro_items) - 1, cache: &cache).__packed)
            }
        }
        values.items = _arr_items
        arenaOfDocument[idx] = values
        return DocumentFrozenGraph.Document(__packed: UInt64(idx), __graph: self)
    }

    private func _restoreDocumentFrozenPacked(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> DocumentFrozenGraph.Document<DocumentFrozenGraph.Arena<Brand>> {
        if let idx = cache[start] { return DocumentFrozenGraph.Document(__packed: UInt64(idx), __graph: self) }
        let idx = arenaOfDocument.count
        cache[start] = idx
        arenaOfDocument.append(DocumentFrozenGraph.DocumentValues())
        ...
        if _obs0 & 4 != 0 {
            ...
            values.meta = try _restoreDocumentMetaFrozenPacked(from: data, at: _cur, cache: &cache).__packed
        ...
        for _ in 0..<Int(_cnt_items) {
            ...
            _arr_items.append(try _restoreDocumentItemFrozenPacked(from: data, at: _cur, cache: &cache).__packed)
