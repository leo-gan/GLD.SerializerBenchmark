...
| [dagr-frozen](https://codeberg.org/mzaks/dagr) | Schema | generated TS (`dagr build`) | arena (built in `prepare()`) → `<Graph>_serde.writeInto` a reused `Builder` / lazy `<Type>Accessor.lazyRoot` → domain |
...
| [dagr-packed](https://codeberg.org/mzaks/dagr) | Schema | generated TS (`dagr build`) | direct builder `writeInto` a reused `Builder` / lazy `<Type>Accessor.lazyRoot` → domain |
| [dagr-regular](https://codeberg.org/mzaks/dagr) | Schema | generated TS (`dagr build`) | arena (built in `prepare()`) → `<Graph>_serde.writeInto` a reused `Builder` / lazy `<Type>Accessor.lazyRoot` → domain |
