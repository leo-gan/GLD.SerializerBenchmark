...
pub mod direct {
    ...
    impl<'a> EventAttr<'a> {
        pub fn store_packed<B: PackedSink + ?Sized>(&self, b: &mut B) -> Result<NodeStoreRef, DagrError> {
            ...
            if let Some(_s) = self.value.as_deref() { let _bs = _s.as_bytes(); b.store_raw(_bs)?; b.store_leb(_bs.len() as u64)?; b.store_leb((1u64 << 1) | 1)?; }
            if let Some(_s) = self.key.as_deref() { let _bs = _s.as_bytes(); b.store_raw(_bs)?; b.store_leb(_bs.len() as u64)?; b.store_leb((0u64 << 1) | 1)?; }
    ...
    impl<'a> Event<'a> {
        pub fn store_packed<B: PackedSink + ?Sized>(&self, b: &mut B) -> Result<NodeStoreRef, DagrError> {
            ...
            if let Some(_s) = self.producer.as_deref() { let _bs = _s.as_bytes(); b.store_raw(_bs)?; b.store_leb(_bs.len() as u64)?; b.store_leb((3u64 << 1) | 1)?; }
            if let Some(_v) = self.occurred_at { let _zz = crate::dagr_runtime::to_zigzag(_v as i64); if crate::dagr_runtime::leb_length(_zz) < 8 { b.store_leb(_zz)?; b.store_leb((2u64 << 1) | 0)?; } else { b.store_i64(_v)?; b.store_leb((2u64 << 1) | 1)?; } }
            if let Some(_s) = self.event_type.as_deref() { let _bs = _s.as_bytes(); b.store_raw(_bs)?; b.store_leb(_bs.len() as u64)?; b.store_leb((1u64 << 1) | 1)?; }
            if let Some(_s) = self.event_id.as_deref() { let _bs = _s.as_bytes(); b.store_raw(_bs)?; b.store_leb(_bs.len() as u64)?; b.store_leb((0u64 << 1) | 1)?; }
            b.store_leb((b.cursor() - _before) as u64)?;
            Ok(NodeStoreRef::Offset(b.cursor()))
        }
    }
