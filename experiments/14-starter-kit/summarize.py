#!/usr/bin/env python3
"""Turn Experiment 14 CSVs into tables people can read and JSON a dashboard can load.

Rebuild from saved CSVs (no new timing run):

    cd analysis
    uv run python ../experiments/14-starter-kit/summarize.py --all

One language:

    uv run python ../experiments/14-starter-kit/summarize.py \\
...
def write_language_markdown(language: str, pack: dict) -> str:
    ...
    lines = [
        f"# Experiment 14 results — {language}",
        "",
    ...
    if similar or close:
        ...
        lines.append(
            ...
            "the fastest library in this starter kit? A faster row is not "
            "automatically the right public format."
        )
        lines.append("")
        if similar:
            lines.append(
        ...
        if front:
            lines.append(
                "**Not both slower and larger than another library in the kit:** "
...
def write_combined_markdown(cfg: dict, languages: dict) -> str:
    ...
    lines = [
        ...
        "cannot tell the library apart from the fastest library in this "
        "starter kit on this sample. **Close** means a small gap. "
        "Read the **role** on each row: that is the job, not a ranking.",
        "",
        "## At a glance",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "Every listed library (public JSON, MessagePack, Protocol Buffers). "
            "Times are middle values in microseconds. Lower is better **inside that language**. "
            "A faster row is not automatically the right public format.",
            "",
        ]
    )
    main_io = analysis.get("main_io") or "memory"
    ...
    for lang, pack in languages.items():
        ...
        if not rows:
            lines.extend(["No in-memory rows.", ""])
            continue
        lines.extend(
            [
