#!/usr/bin/env bash
# Experiment 11 — run one language or every language.
# Does not refresh the published website tables.
#
#   ./experiments/11-memory-vs-stream/run.sh
#   ./experiments/11-memory-vs-stream/run.sh java go
set -euo pipefail
...
run_one() {
  ...
  if [[ -z "$script" ]]; then
    echo "[exp-11] unknown language: $lang" >&2
    return 1
  fi
  if [[ ! -f "$script" ]]; then
    echo "[exp-11] missing runner: $script" >&2
    return 1
  fi
  ...
  if LOG_DIR="$log_dir" BENCHMARK_RUN_CONFIG="$HERE/run.yaml" \
    ...
    if [[ -z "$csv" ]]; then
      echo "[exp-11] $lang: runner finished but no CSV" >&2
      return 1
    fi
    (
      cd "$REPO/analysis"
    ...
    echo "[exp-11] $lang: runner failed" >&2
    return 1
  fi
}
...
if [[ ${#failed[@]} -gt 0 ]]; then
  echo "[exp-11] failed: ${failed[*]}" >&2
  exit 1
fi
echo "[exp-11] all requested languages finished"
