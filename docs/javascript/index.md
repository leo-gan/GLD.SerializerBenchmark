...
- Dagr codegen: `dagr build` in `schemas/v2/dagr/` emits TypeScript into `javascript/src/generated/dagr/`; `npm run generate:dagr` bundles it (esbuild) into `src/generated/dagr_bundle.js`
...
| [dagr](https://codeberg.org/mzaks/dagr) | Schema | generated TS (`dagr build`) | direct builder `writeInto` a reused `Builder` / lazy `<Type>Accessor.lazyRoot` → domain |
| [dagr-frozen](https://codeberg.org/mzaks/dagr) | Schema | generated TS (`dagr build`) | arena (built in `prepare()`) → `<Graph>_serde.toBytes` / lazy `<Type>Accessor.lazyRoot` → domain |
| [dagr-frozen-packed](https://codeberg.org/mzaks/dagr) | Schema | generated TS (`dagr build`) | direct builder `writeInto` a reused `Builder` / lazy `<Type>Accessor.lazyRoot` → domain |
| [dagr-regular](https://codeberg.org/mzaks/dagr) | Schema | generated TS (`dagr build`) | arena (built in `prepare()`) → `<Graph>_serde.toBytes` / lazy `<Type>Accessor.lazyRoot` → domain |
...
- **dagr** uses TypeScript generated by `dagr build` from `schemas/v2/dagr/schema.py` (one graph per suite type, all nodes `packed`), bundled to plain ESM with esbuild. Serialize runs only `writeInto` on direct-builder value objects built in `prepare()` (i64 fields → `BigInt`), as protobufjs / protobuf-es build their messages there; the generated `writeInto` stores into one `Builder` reused across calls (`reset()`), and batch records are copied straight into a reused output buffer. Deserialize runs the lazy reader and materializes the domain value inside the timed path. N>1 is framed `[u32 count][u32 len][record]…`.
- **dagr-regular / dagr-frozen / dagr-frozen-packed** are the same schema in the other three node layouts (`<Type>RegularGraph`, `<Type>FrozenGraph`, `<Type>FrozenPackedGraph`), sharing the harness code and batch framing with `dagr`. `dagr-frozen-packed` encodes exactly like `dagr` (direct builder, reused `Builder`). `dagr-regular` and `dagr-frozen` have no direct builder: `prepare()` builds one generated arena per record (the native model, like the protobuf messages) and the timed serialize runs only `<Graph>_serde.toBytes`, which allocates its own `Builder` per call (the generated API has no reuse entry point). Decode is the same lazy-reader → domain path for all four.
