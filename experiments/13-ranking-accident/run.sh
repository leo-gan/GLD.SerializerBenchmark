#!/usr/bin/env bash
# Experiment 13 — run one language or every language.
# Does not refresh the published website tables.
#
#   ./experiments/13-ranking-accident/run.sh
#   ./experiments/13-ranking-accident/run.sh java go
set -euo pipefail
...
run_one() {
  ...
  if [[ -z "$script" ]]; then
    echo "[exp-13] unknown language: $lang" >&2
    return 1
  fi
  if [[ ! -f "$script" ]]; then
    echo "[exp-13] missing runner: $script" >&2
    return 1
  fi
  ...
  if LOG_DIR="$log_dir" BENCHMARK_RUN_CONFIG="$HERE/run.yaml" \
    ...
    if [[ -z "$csv" ]]; then
      echo "[exp-13] $lang: runner finished but no CSV" >&2
      return 1
    fi
    (
      cd "$REPO/analysis"
    ...
    echo "[exp-13] $lang: runner failed" >&2
    return 1
  fi
}
...
echo "[exp-13] all requested languages finished"
