...
pub use crate::strings_frozen_packed_graph_core::*;
...
// ── StringsFrozenPackedGraphGraph ─────────────────────────────────────────────────────────────────────
pub trait StringsFrozenPackedGraphGraph: StringsArena {
...
impl<T: StringsArena> StringsFrozenPackedGraphGraph for T {}

// ── Strings handle ────────────────────────────────────────────────────────
pub struct Strings<'arena, G: StringsFrozenPackedGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: StringsFrozenPackedGraphGraph> Clone for Strings<'arena, G> {
    fn clone(&self) -> Self { Strings { index: self.index, graph: self.graph } }
}
impl<'arena, G: StringsFrozenPackedGraphGraph> Copy for Strings<'arena, G> {}

impl<'arena, G: StringsFrozenPackedGraphGraph> Strings<'arena, G> {
    ...
    fn _cycle_eq<B: StringsFrozenPackedGraphGraph>(&self, other: &Strings<'_, B>,
...
impl<'a, 'b, A: StringsFrozenPackedGraphGraph, B: StringsFrozenPackedGraphGraph> PartialEq<Strings<'b, B>> for Strings<'a, A> {
    fn eq(&self, other: &Strings<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: StringsFrozenPackedGraphGraph> Eq for Strings<'arena, G> {}

impl<'arena, G: StringsFrozenPackedGraphGraph> std::hash::Hash for Strings<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: StringsFrozenPackedGraphGraph> fmt::Display for Strings<'arena, G> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut v = HashSet::new(); self._fmt_with(f, &mut v)
    }
}

// ── StringsFrozenPackedGraphArena<const ID: u64> ─────────────────────────────────────────────────────
pub struct StringsFrozenPackedGraphArena<const ID: u64> {
    arena_of_strings: RefCell<Vec<StringsValues>>,
    root: Cell<Option<NodeRef>>,
}

impl<const ID: u64> StringsArena for StringsFrozenPackedGraphArena<ID> {
...
impl<const ID: u64> StringsFrozenPackedGraphArena<ID> {
    pub fn new() -> Self {
        StringsFrozenPackedGraphArena {
...
impl<const ID: u64> Default for StringsFrozenPackedGraphArena<ID> {
...
fn _restore_strings<'arena, G: StringsFrozenPackedGraphGraph>(
    ...
    let _items_val = if _obs0 & 1 != 0 {
        ...
        _arr_items = crate::dagr_runtime::PackedStrArray::new(data, _cur - _clb_items)?.iter().map(|r| r.map(|s| s.to_string())).collect::<Result<Vec<_>, _>>()?;
        _cur = _arr_end_items;
        _cur = _arr_end_items;
        _arr_items
    } else { vec![] };
