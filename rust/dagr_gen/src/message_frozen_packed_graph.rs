...
pub use crate::message_frozen_packed_graph_core::*;
...
// ── MessageFrozenPackedGraphGraph ─────────────────────────────────────────────────────────────────────
pub trait MessageFrozenPackedGraphGraph: MessageArena {
...
impl<T: MessageArena> MessageFrozenPackedGraphGraph for T {}

// ── Message handle ────────────────────────────────────────────────────────
pub struct Message<'arena, G: MessageFrozenPackedGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: MessageFrozenPackedGraphGraph> Clone for Message<'arena, G> {
    fn clone(&self) -> Self { Message { index: self.index, graph: self.graph } }
}
impl<'arena, G: MessageFrozenPackedGraphGraph> Copy for Message<'arena, G> {}

impl<'arena, G: MessageFrozenPackedGraphGraph> Message<'arena, G> {
    ...
    fn _cycle_eq<B: MessageFrozenPackedGraphGraph>(&self, other: &Message<'_, B>,
...
impl<'a, 'b, A: MessageFrozenPackedGraphGraph, B: MessageFrozenPackedGraphGraph> PartialEq<Message<'b, B>> for Message<'a, A> {
    fn eq(&self, other: &Message<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: MessageFrozenPackedGraphGraph> Eq for Message<'arena, G> {}

impl<'arena, G: MessageFrozenPackedGraphGraph> std::hash::Hash for Message<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: MessageFrozenPackedGraphGraph> fmt::Display for Message<'arena, G> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut v = HashSet::new(); self._fmt_with(f, &mut v)
    }
}

// ── MessageFrozenPackedGraphArena<const ID: u64> ─────────────────────────────────────────────────────
pub struct MessageFrozenPackedGraphArena<const ID: u64> {
    arena_of_message: RefCell<Vec<MessageValues>>,
    root: Cell<Option<NodeRef>>,
}

impl<const ID: u64> MessageArena for MessageFrozenPackedGraphArena<ID> {
...
impl<const ID: u64> MessageFrozenPackedGraphArena<ID> {
    pub fn new() -> Self {
        MessageFrozenPackedGraphArena {
...
impl<const ID: u64> Default for MessageFrozenPackedGraphArena<ID> {
...
fn _restore_message<'arena, G: MessageFrozenPackedGraphGraph>(
    ...
    let _f_float64_val = if _obs0 & 8 != 0 {
        let _rv = if _ebs0 & 4 != 0 {
            let _v = crate::dagr_runtime::read_f64(data, _cur)?; _cur += 8; _v
        } else {
            let (_v, _vb) = crate::dagr_runtime::read_packed_f64(data, _cur)?; _cur += _vb; _v
        };
        Some(_rv)
    } else { None };
    let _f_string_val = if _obs0 & 16 != 0 {
        let (_sv_f_string, _slb_f_string) = crate::dagr_runtime::read_leb(data, _cur)?;
        _cur += _slb_f_string;
        let _s_f_string = String::from_utf8_lossy(data.get(_cur.._cur + _sv_f_string as usize).ok_or(DagrError::InvalidData)?).into_owned();
        _cur += _sv_f_string as usize;
        Some(_s_f_string)
    } else { None };
    ...
    let _f_string_2_val = if _obs0 & 128 != 0 {
        let (_sv_f_string_2, _slb_f_string_2) = crate::dagr_runtime::read_leb(data, _cur)?;
        _cur += _slb_f_string_2;
        let _s_f_string_2 = String::from_utf8_lossy(data.get(_cur.._cur + _sv_f_string_2 as usize).ok_or(DagrError::InvalidData)?).into_owned();
        _cur += _sv_f_string_2 as usize;
        Some(_s_f_string_2)
    } else { None };
