...
pub fn stringifyFixture(fx: data.Fixture, out: *buf_mod.Buf) !void {
    var aw = out.takeWriter();
    defer out.restoreWriter(&aw);
    switch (fx) {
        inline else => |payload| {
...
pub fn parseFixture(allocator: std.mem.Allocator, type_id: []const u8, bytes: []const u8) !data.Fixture {
    return parseFixtureWith(allocator, type_id, bytes, .alloc_always);
}

/// Unescaped strings come back as views into `bytes`, so the input must stay
/// alive and unmodified for as long as the fixture is read. Escaped strings
/// and every container still use `allocator`.
pub fn parseFixtureBorrowed(allocator: std.mem.Allocator, type_id: []const u8, bytes: []const u8) !data.Fixture {
    return parseFixtureWith(allocator, type_id, bytes, .alloc_if_needed);
}
...
fn parseFixtureWith(
    ...
    type_id: []const u8,
    bytes: []const u8,
    comptime allocate: std.json.AllocWhen,
) !data.Fixture {
