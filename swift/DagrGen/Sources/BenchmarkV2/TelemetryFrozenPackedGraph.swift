...
public enum TelemetryFrozenPackedGraph {
    ...
    public typealias TelemetryFrozenPackedGraphGraph = TelemetryArena
    ...
    //
    //   enum MyBrand {}
    //   let arena = TelemetryFrozenPackedGraph.Arena<MyBrand>()
    //
...
extension TelemetryFrozenPackedGraph.Arena {
    public func newTelemetry(source: String? = nil, ts: Int64? = nil, tags: [String] = [], values: [Double] = []) -> TelemetryFrozenPackedGraph.Telemetry<TelemetryFrozenPackedGraph.Arena<Brand>> {
        let _idx: Int
        _idx = arenaOfTelemetry.count
        arenaOfTelemetry.append(TelemetryFrozenPackedGraph.TelemetryValues(source: source, ts: ts, tags: tags, values: values))
        let _packed = UInt64(_idx)
        return TelemetryFrozenPackedGraph.Telemetry(__packed: _packed, __graph: self)
    }
}

extension TelemetryFrozenPackedGraph.Arena {
    public func adopt<S: TelemetryFrozenPackedGraph.TelemetryGraph>(_ src: TelemetryFrozenPackedGraph.Telemetry<S>) throws -> TelemetryFrozenPackedGraph.Telemetry<TelemetryFrozenPackedGraph.Arena<Brand>> {
        var _seen = Set<UInt64>()
        return try _adopt(src, &_seen)
    }
    func _adopt<S: TelemetryFrozenPackedGraph.TelemetryGraph>(_ src: TelemetryFrozenPackedGraph.Telemetry<S>, _ _seen: inout Set<UInt64>) throws -> TelemetryFrozenPackedGraph.Telemetry<TelemetryFrozenPackedGraph.Arena<Brand>> {
...
extension TelemetryFrozenPackedGraph.Telemetry: CustomStringConvertible {
...
extension TelemetryFrozenPackedGraph.Telemetry: Hashable {
...
extension TelemetryFrozenPackedGraph.Telemetry: Equatable {
...
extension TelemetryFrozenPackedGraph.Telemetry {
    public static func == <OtherArena: TelemetryFrozenPackedGraph.TelemetryGraph>(lhs: Self, rhs: TelemetryFrozenPackedGraph.Telemetry<OtherArena>) -> Bool {
        var visited = Set<ArenaPair>()
    ...
    func cycleAwareEqualsAny<OtherArena: TelemetryFrozenPackedGraph.TelemetryGraph>(
        other: TelemetryFrozenPackedGraph.Telemetry<OtherArena>,
...
extension TelemetryFrozenPackedGraph.Telemetry: ArenaNodeHandle {}
extension TelemetryFrozenPackedGraph.Telemetry: ArenaGraphStorable {
...
extension TelemetryFrozenPackedGraph.Arena {
    ...
    public static func restore(from data: Foundation.Data) throws -> TelemetryFrozenPackedGraph.Arena<Brand> {
        guard !data.isEmpty else { return TelemetryFrozenPackedGraph.Arena<Brand>() }
        let arena = TelemetryFrozenPackedGraph.Arena<Brand>()
    ...
    private func _restoreTelemetry(from data: Foundation.Data, at start: Int, cache: inout [Int: Int]) throws -> TelemetryFrozenPackedGraph.Telemetry<TelemetryFrozenPackedGraph.Arena<Brand>> {
        if let idx = cache[start] { return TelemetryFrozenPackedGraph.Telemetry(__packed: UInt64(idx), __graph: self) }
        let idx = arenaOfTelemetry.count
        cache[start] = idx
        arenaOfTelemetry.append(TelemetryFrozenPackedGraph.TelemetryValues())
        var values = TelemetryFrozenPackedGraph.TelemetryValues()
        ...
        return TelemetryFrozenPackedGraph.Telemetry(__packed: UInt64(idx), __graph: self)
    }
