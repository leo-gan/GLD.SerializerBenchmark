...
Rebuild from saved CSVs (no new timing run):
    ...
    cd analysis
    uv run python ../experiments/13-ranking-accident/summarize.py --all

One language:

    uv run python ../experiments/13-ranking-accident/summarize.py \\
...
def write_language_markdown(language: str, pack: dict) -> str:
    lines = [
        f"# Experiment 13 results — {language}",
        "",
