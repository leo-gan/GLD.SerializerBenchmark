"""Dagr (Data Graph), `frozen+packed` layout, on the suite types — row `dagr-frozen-packed`.

Schema: `schemas/v2/dagr/schema.py` -> `dagr build` -> `src/gen/dagr/` (the
`<Type>FrozenPackedGraph` graphs: every node `frozen=True, packed=True` — presence bitset +
...
from message_frozen_packed_graph_direct import DirectMessage, write_message_graph_direct
from strings_frozen_packed_graph_direct import DirectStrings, write_strings_graph_direct
from document_frozen_packed_graph_direct import (
    DirectDocument,
    DirectDocumentItem,
    DirectDocumentMeta,
    write_document_graph_direct,
)
from telemetry_frozen_packed_graph_direct import DirectTelemetry, write_telemetry_graph_direct
from event_frozen_packed_graph_direct import DirectEvent, DirectEventAttr, write_event_graph_direct
from message_frozen_packed_graph_reader import read_message_root
from strings_frozen_packed_graph_reader import read_strings_root
from document_frozen_packed_graph_reader import read_document_root
from telemetry_frozen_packed_graph_reader import read_telemetry_root
from event_frozen_packed_graph_reader import read_event_root
...
# Every frozen+packed module uses `_VT_MAX = 1` (no vtables), so one `Builder[1]` serves
# all five graphs.
comptime _VT = 1
...
struct DagrFrozenPackedSer(Movable):
    var version: String
    var _b: Builder[_VT]
    ...
    def serialize_bytes(mut self, fx: Fixture) raises -> List[Byte]:
        ...
        if fx.type_id == "message":
            ...
            raise Error("dagr-frozen-packed: unsupported type " + fx.type_id)
        self._hint = len(out)
        return out^
    ...
    def deserialize_bytes(self, fx: Fixture, data: List[Byte]) raises -> Fixture:
        ...
        if n != fx.n:
            raise Error("dagr-frozen-packed: batch count " + String(n) + " != " + String(fx.n))
        var o = 4
        for _ in range(n):
            var ln = _get_u32(buf, o)
            o += 4
            if o + ln > len(buf):
                raise Error("dagr-frozen-packed: truncated batch payload")
