...
Implement a **deliberately small** Protobuf binary encoder/decoder for a teaching message, validate against **golden bytes** and (recommended) an official parser. You will practice the wire rules from [Protobuf wire format](protobuf-wire-format.md)—not rebuild `protoc`.
...
- [ ] Varint encode/decode (unsigned)  
- [ ] LEN fields: key, len varint, payload  
...
function encode_mini_user(u):
  ...
  if u.manager is not null:
    inner = encode_mini_user(u.manager)  # temporary buffer
...
Your encoder may omit default fields; decoders must accept any field order.
...
function decode_mini_user(buf):
  ...
  while i < len(buf):
    ...
    if wt == 0:
      ...
      # else skip
    elif wt == 1: i += 8   # skip fixed64
    elif wt == 5: i += 4   # skip fixed32
    else: error
  return user
```
...
### 2. Official parser (recommended)

Minimal `.proto` + Python:
...
protobuf-c or prost work equally as oracles.

### 3. Round-trip

`decode(encode(x))` equals `x` for a small property set (ids, names, one level of manager, a few tags).

## Extension ideas

- Packed `repeated uint32` (single LEN field, concatenated varints).  
- `uint64` / fixed32.  
- Reject overlong varints / remaining bytes after nested len.  
...
- Skipping unknowns is part of being a real decoder.  
