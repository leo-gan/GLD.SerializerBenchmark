...
impl<T: StringsArena> StringsGraphGraph for T {}
...
impl<'arena, G: StringsGraphGraph> Strings<'arena, G> {
    ...
    fn _cycle_eq<B: StringsGraphGraph>(&self, other: &Strings<'_, B>,
...
impl<'a, 'b, A: StringsGraphGraph, B: StringsGraphGraph> PartialEq<Strings<'b, B>> for Strings<'a, A> {
    fn eq(&self, other: &Strings<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: StringsGraphGraph> Eq for Strings<'arena, G> {}

impl<'arena, G: StringsGraphGraph> std::hash::Hash for Strings<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: StringsGraphGraph> fmt::Display for Strings<'arena, G> {
...
impl<const ID: u64> StringsArena for StringsGraphArena<ID> {
...
impl<const ID: u64> Default for StringsGraphArena<ID> {
...
fn _restore_strings<'arena, G: StringsGraphGraph>(
        ...
        match _field_idx {
            0 => {
                ...
                for _ in 0.._count {
                    ...
                    _items_val.push(crate::dagr_runtime::read_string(data, _ap)?);
    ...
    let _node = arena.new_strings(_items_val);
