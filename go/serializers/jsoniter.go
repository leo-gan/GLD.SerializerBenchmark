...
func (s *jsonIter) Prepare(fx data.Fixture) error {
	s.proto = fx.Value
	return nil
}

func (s *jsonIter) SerializeBytes(fx data.Fixture) ([]byte, error) {
...
func (s *jsonIter) SerializeStream(fx data.Fixture, w io.Writer) (int, error) {
	cw := &countWriter{w: w}
