...
// ── TelemetryFrozenGraphGraph ─────────────────────────────────────────────────────────────────────
pub trait TelemetryFrozenGraphGraph: TelemetryArena {
...
impl<T: TelemetryArena> TelemetryFrozenGraphGraph for T {}

// ── Telemetry handle ────────────────────────────────────────────────────────
pub struct Telemetry<'arena, G: TelemetryFrozenGraphGraph> {
    pub(crate) index: u32,
    pub(crate) graph: &'arena G,
}

impl<'arena, G: TelemetryFrozenGraphGraph> Clone for Telemetry<'arena, G> {
    fn clone(&self) -> Self { Telemetry { index: self.index, graph: self.graph } }
}
impl<'arena, G: TelemetryFrozenGraphGraph> Copy for Telemetry<'arena, G> {}

impl<'arena, G: TelemetryFrozenGraphGraph> Telemetry<'arena, G> {
    ...
    fn _cycle_eq<B: TelemetryFrozenGraphGraph>(&self, other: &Telemetry<'_, B>,
...
impl<'a, 'b, A: TelemetryFrozenGraphGraph, B: TelemetryFrozenGraphGraph> PartialEq<Telemetry<'b, B>> for Telemetry<'a, A> {
    fn eq(&self, other: &Telemetry<'b, B>) -> bool {
        let mut v = HashSet::new(); self._cycle_eq(other, &mut v)
    }
}
impl<'arena, G: TelemetryFrozenGraphGraph> Eq for Telemetry<'arena, G> {}

impl<'arena, G: TelemetryFrozenGraphGraph> std::hash::Hash for Telemetry<'arena, G> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut v = HashSet::new(); self._hash_with(state, &mut v);
    }
}

impl<'arena, G: TelemetryFrozenGraphGraph> fmt::Display for Telemetry<'arena, G> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut v = HashSet::new(); self._fmt_with(f, &mut v)
    }
}

// ── TelemetryFrozenGraphArena<const ID: u64> ─────────────────────────────────────────────────────
pub struct TelemetryFrozenGraphArena<const ID: u64> {
    arena_of_telemetry: RefCell<Vec<TelemetryValues>>,
    root: Cell<Option<NodeRef>>,
}

impl<const ID: u64> TelemetryArena for TelemetryFrozenGraphArena<ID> {
...
impl<const ID: u64> TelemetryFrozenGraphArena<ID> {
    pub fn new() -> Self {
        TelemetryFrozenGraphArena {
...
impl<const ID: u64> Default for TelemetryFrozenGraphArena<ID> {
...
impl<'arena, G: TelemetryFrozenGraphGraph> Telemetry<'arena, G> {
    pub fn store(&self, b: &mut DagrBuilder) -> Result<NodeStoreRef, DagrError> {
        ...
        if let Some(off) = _values_off { b.store_forward_pointer(off); }
        if let Some(off) = _tags_off { b.store_forward_pointer(off); }
        if let Some(_v_ts) = self.ts() { b.store_i64(_v_ts); }
        if let Some(off) = _source_off { b.store_forward_pointer(off); }
        b.store_u8(u8::from(_pr.source.is_some()) | (u8::from(self.ts().is_some()) << 1) | (u8::from(!_pr.tags.is_empty()) << 2) | (u8::from(!_pr.values.is_empty()) << 3));
...
fn _restore_telemetry<'arena, G: TelemetryFrozenGraphGraph>(
    ...
    let _tags_val = if _cur < data.len() {
        let (_fwd_raw, _fwd_bs) = crate::dagr_runtime::read_v62(data, _cur)?;
        let _at_tags = _cur + _fwd_bs + _fwd_raw as usize;
        _cur += _fwd_bs;
        crate::dagr_runtime::read_value_ref_array(data, _at_tags)?.into_iter()
            .filter_map(|p| p.map(|a| crate::dagr_runtime::read_string(data, a).unwrap_or_default()))
            .collect()
    } else { vec![] };
    let _values_val = if _cur < data.len() {
        let (_fwd_raw, _fwd_bs) = crate::dagr_runtime::read_v62(data, _cur)?;
        let _at_values = _cur + _fwd_bs + _fwd_raw as usize;
        _cur += _fwd_bs;
        crate::dagr_runtime::read_inline_prim_array_native::<f64>(data, _at_values)?
    } else { vec![] };
